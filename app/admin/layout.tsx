...
export default async function AdminLayout({ children }: AdminLayoutProps) {
  ...
  return (
    <div className="flex h-screen flex-col md:flex-row bg-gray-50">
      {/* Sidebar (Desktop) */}
      <aside className="hidden w-64 flex-col border-r bg-white md:flex pl-4 pr-4 pt-6 pb-6">
        ...
        <nav className="flex flex-col gap-2 flex-1">
          {navLinks.map((link) => {
            ...
            // A simple rough check for active links. Let's keep it simple.
            return (
              <Link
                key={link.name}
                href={link.href}
      ...
      {/* Main Content Area */}
      <main className="flex-1 overflow-y-auto w-full md:pb-0 pb-24">
